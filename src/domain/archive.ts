...
export function tarExtract(bytes: Uint8Array): TarExtractResult {
  ...
  let count = 0
  let declared = 0
  // GNU longname ('L') and PAX per-file extended header ('x') override only
  // the very next entry's name. A PAX *global* header ('g') is meant to apply
  // to every entry from that point on until superseded by a later 'g' (or the
  // archive ends) — tracked separately so a per-entry 'L'/'x' can still
  // override it for one entry without clobbering the standing global value.
  ...
  while (off + BLOCK <= bytes.length) {
    ...
    if (typeflag === 'g') {
      const path = parsePaxHeader(data).path
      globalName = path || undefined
      continue
    }

    const entryName = pendingName ?? globalName ?? name

...
describe('http adapter', () => {
  ...
  it('POST /op/run: trace: "full" additionally attaches inputRef/outputRef Handle snapshots to the trace', async () => {
    const res = await post('op/run', {
      spec: { tag: 'leaf', name: 'convert' },
      input: { handle: { $handle: true, base64: b64('{"a":1}'), type: 'application/json' }, from: 'json', to: 'yaml' },
      trace: 'full',
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as { result: { base64: string }; trace: Array<{ kind: string; inputRef?: { base64: string }; outputRef?: { base64: string } }> }
    expect(atob(body.result.base64)).toBe('a: 1')

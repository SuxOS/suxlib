...
test('runInline forwards gOpts to runGoverned: onEvent fires and custom sleep/rand are honored', async () => {
  ...
  expect(events).toEqual([
    { kind: 'retry-attempt', name: 'flaky', attempt: 0, delayMs: expect.any(Number), runId: expect.any(String) },
    { kind: 'retry-attempt', name: 'flaky', attempt: 1, delayMs: expect.any(Number), runId: expect.any(String) },
  ])

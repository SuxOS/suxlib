...
/** Build a bare tar header + body block, for hand-assembling multi-entry archives (global PAX header tests). */
function buildTarBlock(name: string, size: number, tflag: string, body: Uint8Array): Uint8Array {
  const h = new Uint8Array(BLOCK)
  h.set(strToU8(name.slice(0, 100)), 0)
  h.set(octalField(size, 12), 124)
  h[156] = tflag.charCodeAt(0)
  ...
  for (let i = 0; i < BLOCK; i++) checksum += h[i]
  h.set(octalField(checksum, 8), 148)
  const paddedBody = new Uint8Array(padTo(size, BLOCK))
  paddedBody.set(body, 0)
  return new Uint8Array([...h, ...paddedBody])
}
...
test('tarExtract applies a PAX global header ("g" typeflag) to every following entry until superseded', () => {
  const globalName = 'c/'.repeat(60) + 'deep/global.txt'
  const overrideName = 'd/'.repeat(60) + 'deep/override.txt'
  const paxGlobalBody = strToU8(paxRecord('path', globalName))
  const paxGlobalBody2 = strToU8(paxRecord('path', overrideName))

  const parts = [
    buildTarBlock('PaxHeader', paxGlobalBody.length, 'g', paxGlobalBody),
    buildTarBlock('first.txt', 3, '0', strToU8('AAA')),
    buildTarBlock('second.txt', 3, '0', strToU8('BBB')),
    buildTarBlock('PaxHeader', paxGlobalBody2.length, 'g', paxGlobalBody2),
    buildTarBlock('third.txt', 3, '0', strToU8('CCC')),
  ...
  const { entries, skipped } = tarExtract(packed)
  expect(skipped).toEqual([])
  expect(entries.map((e) => e.name)).toEqual([globalName, globalName, overrideName])
})

test('tarExtract lets a per-entry "L"/"x" name override a still-active PAX global header for one entry only', () => {
  const globalName = 'e/'.repeat(60) + 'deep/global.txt'
  const perEntryName = 'f/'.repeat(60) + 'deep/one-off.txt'
  const paxGlobalBody = strToU8(paxRecord('path', globalName))
  const paxOverrideBody = strToU8(paxRecord('path', perEntryName))

  const parts = [
    buildTarBlock('PaxHeader', paxGlobalBody.length, 'g', paxGlobalBody),
    buildTarBlock('PaxHeader', paxOverrideBody.length, 'x', paxOverrideBody),
    buildTarBlock(perEntryName.slice(0, 100), 3, '0', strToU8('AAA')),
    buildTarBlock('after.txt', 3, '0', strToU8('BBB')),
    new Uint8Array(BLOCK * 2),
  ]
  const packed = new Uint8Array(parts.reduce((n, p) => n + p.length, 0))
  let off = 0
  ...
  const { entries, skipped } = tarExtract(packed)
  expect(skipped).toEqual([])
  expect(entries.map((e) => e.name)).toEqual([perEntryName, globalName])
})

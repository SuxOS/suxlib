...
test('htmlToMarkdown keeps trailing text after a nested <blockquote> instead of truncating it (#411)', () => {
  const md = htmlToMarkdown('<blockquote>Outer<blockquote>Inner</blockquote>tail</blockquote>')
  expect(md).toBe('> Outer\n> > Inner\n> tail')
})

test('htmlToMarkdown handles sibling blockquotes each independently, including one with no nesting', () => {
  const md = htmlToMarkdown('<blockquote>A</blockquote><blockquote>B<blockquote>B1</blockquote></blockquote>')
  expect(md).toBe('> A\n\n> B\n> > B1')
})
